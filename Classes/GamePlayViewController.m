...
- (void)initializeEmulatorView {
	...
	if (UIInterfaceOrientationIsLandscape(orientation) == YES) {
        ...
        //surfaceRect = CGRectMake(0.0, 0.0, emuWidth, emuHeight);
        NSLog(@"screen width: %f height: %f", screenWidth, screenHeight);
        
        surfaceRect = CGRectMake((screenHeight - emuWidth) / 2.0, (screenWidth - emuHeight) / 2.0, emuWidth, emuHeight);
	...
	if (UIInterfaceOrientationIsLandscape(orientation) == YES) {
        ...
        controllerView = [ [ ControllerView alloc ] initWithFrame: CGRectMake((screenHeight - h)/2.0, (screenWidth - 320.0) / 2.0, h, 320.0) ];
	} else {
